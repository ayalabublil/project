...
import { getManagers } from '../service/manager.js';
...
export class Manager {
    ...
    get=( async (req, res) => {
        try {
            const password = req.params.password; // קבלת קוד מהפרמטרים

            // שאילתא לקבלת מנהל לפי קוד
            const result = await pool.request()
                .input('password', sql.Int, t_z) // הנח שהת"ז היא מספר שלם
                .query('SELECT * FROM users WHERE t_z = @t_z');

            const user = result.recordset[0]; // קבלת המשתמש הראשון

            if (!user) {
                return res.status(404).send("User not found");
            }

            res.status(200).send(user); // החזרת המשתמש
